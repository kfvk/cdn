fm.liveswitch.Util.addOnLoad(function () {
    ...
    var sendEncodingsHighCheckbox = document.getElementById('sendEncodingHigh');
    var sendEncodingsMidCheckbox = document.getElementById('sendEncodingMid');
    var sendEncodingsLowCheckbox = document.getElementById('sendEncodingLow');
    var receiveEncodingsGroup = document.getElementById('receiveEncodingsGroup');
    var receiveEncodingsList = document.getElementById('receiveEncodingsList');
    ...
    var start = function (channelId) {
        ...
        */
        var simulcastHash = getHashParameter('simulcast');
        var simulcast = (simulcastHash == '1' || simulcastHash == 'true' || simulcastHash == 'yes');
        var screencapture;// = screencaptureCheckbox.checked;
        //if (screencapture) {
            setHashParameter('screencapture', '0');
        //}
        ...
        app.startLocalMedia(video, screencapture, audioonly, receiveonly, simulcast, audioDeviceList, videoDeviceList, receiveEncodingsGroup).then(function (o) {
            if (!receiveonly) {
            ...
            app.joinAsync(incomingMessage, peerLeft, peerJoined, clientRegistered).then(function (o) {
                ...
                writeMessage('<b>You\'ve joined The Midnight Commander</b>'; //the ' + app.channelId + ' as ' + nameInput.value + '.</b>');
    ...
    var showSendEncodings = function (show) {
        if (show) {
            sendEncodingsGroup.style.visibility = "visible";
            sendEncodingsHighCheckbox.checked = true;
            sendEncodingsMidCheckbox.checked = true;
            sendEncodingsLowCheckbox.checked = true;
        }
        else {
            sendEncodingsGroup.style.visibility = "collapse";
        }
    };
    // Attach DOM events.
    ...
    */
    ...
    fm.liveswitch.Util.observe(sendEncodingsHighCheckbox, 'click', function (evt) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(sendEncodingsMidCheckbox, 'click', function (evt) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(sendEncodingsLowCheckbox, 'click', function (evt) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(receiveEncodingsList, 'change', function (evt) {
        var option = receiveEncodingsList.options[receiveEncodingsList.selectedIndex];
        app.changeReceiveEncodings(option.id);
    });
    ...
    var channelId = 'The Midnight Commander';//getHashParameter('channel');
    if (channelId) {

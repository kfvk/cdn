...
fm.liveswitch.Util.addOnLoad(() => {
    ...
    let sendEncodingsHighCheckbox = document.getElementById('sendEncodingHigh') as HTMLInputElement;
    let sendEncodingsMidCheckbox = document.getElementById('sendEncodingMid') as HTMLInputElement;
    let sendEncodingsLowCheckbox = document.getElementById('sendEncodingLow') as HTMLInputElement;
    let receiveEncodingsGroup = document.getElementById('receiveEncodingsGroup') as HTMLDivElement;
    let receiveEncodingsList = document.getElementById('receiveEncodingsList') as HTMLSelectElement;
    ...
    let start = (channelId: string) => {
        ...
        */
        let simulcastHash = getHashParameter('simulcast');
        let simulcast = (simulcastHash == '1' || simulcastHash == 'true' || simulcastHash == 'yes');
        ...
        app.startLocalMedia(video, screencapture, audioonly, receiveonly, simulcast, audioDeviceList, videoDeviceList, receiveEncodingsGroup).then((o) => {
            if (!receiveonly) {
    ...
    var showSendEncodings = function (show: boolean) {
        if (show) {
    ...
    */
    ...
    fm.liveswitch.Util.observe(sendEncodingsHighCheckbox, 'click', function (evt: any) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(sendEncodingsMidCheckbox, 'click', function (evt: any) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(sendEncodingsLowCheckbox, 'click', function (evt: any) {
        app.changeSendEncodings(this.value);
    });
    fm.liveswitch.Util.observe(receiveEncodingsList, 'change', function (evt: any) {
        let option = receiveEncodingsList.options[receiveEncodingsList.selectedIndex];
